...
ORDER_FIRST_TIME = 2          # 首单间隔2秒

# 新增优化参数 - 平衡效率与持仓规模
MAX_ORDERS_PER_SIDE = 20        # 单边最大订单数 (Nachkäufe)
ORDER_REFRESH_INTERVAL = 20    # 订单刷新间隔(秒)
PRICE_UPDATE_THRESHOLD = 0.001  # 价格变动阈值 0.1%

# 🚀 动态止盈参数
DYNAMIC_PROFIT_MIN = 0.005    # 最小止盈率 0.5%
DYNAMIC_PROFIT_MAX = 0.1      # 最大止盈率 10%
HEDGE_RATIO_DIVISOR = 100     # 对冲比例除数
INVENTORY_REDUCTION_RATIO = 0.8  # 库存风险阈值比例 (80%)

# 🔧 API优化参数
POSITION_SYNC_INTERVAL = 60   # 持仓同步间隔 (1分钟)
ORDER_SYNC_INTERVAL = 30      # 订单同步间隔 (30秒)
STATS_DISPLAY_INTERVAL = 300  # 统计显示间隔 (5分钟)
LOG_THROTTLE_FACTOR = 10      # 日志节流因子


class GridBot:
    """网格交易机器人 - NUR LONG mit Gewinnziel"""

    def __init__(self, dry_run=False, max_orders_per_side=None, grid_spacing=None, order_amount=None, price_threshold=None, take_profit_percent=None):
        ...
        self.take_profit_percent = take_profit_percent or 1.0  # Standard: 1% Gewinn
        ...
        self.avg_entry_price = 0  # Durchschnittseinstiegspreis
        self.latest_price = 0
        ...
        self.price_update_threshold = price_threshold or PRICE_UPDATE_THRESHOLD
    ...
    async def setup(self):
        ...
        logger.info(f"📊 Strategie: Nachkauf bei -{self.grid_spacing*100:.1f}%, Verkauf bei +{self.take_profit_percent:.1f}% Gewinn")
            ...
            if self.long_position > position_threshold:
                ...
                if counts['sell_orders'] <= 0:
                    ...
                    if self.avg_entry_price > 0:
                        exit_price = self.avg_entry_price * (1 + take_profit_decimal)
                        logger.info(f"🔄 Verkaufe bei {self.take_profit_percent:.1f}% Gewinn (${exit_price:.6f})")
                    else:
                        exit_price = self.latest_price * 1.01
                        logger.info("🔄 Kein Durchschnittspreis - verwende 1%")
                ...
                # 🔥 Nachkauf: Bei fallendem Preis
                entry_price = self.latest_price * (1 - self.grid_spacing)
                
                # 🔥 Verkauf: Erst bei Gewinn (Take-Profit in %)
                take_profit_decimal = self.take_profit_percent / 100.0
    ...
    async def adjust_grid_strategy(self):
        ...
        try:
            ...
            # Preis-Threshold Prüfung (reduziert Updates)
...
async def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='简化 Lighter 网格机器人 - NUR LONG')
    ...
    logger.info(f"   Gewinnziel: {args.take_profit:.1f}%")
...
if __name__ == "__main__":
    print("🤖 简化 Lighter 网格机器人 - NUR LONG")
    print("📊 Kauft bei Dips, verkauft erst bei Gewinn")
    print("=" * 50)
    asyncio.run(main())
